...
Then generate a set of x from uniform distribution U(0,1). Use bandwidth 0.05, 0.5 and 1 respectively and plot the kernel density against x.
...
Run another test using a set of x from normal distribution N(0,1). Use bandwidth 0.05, 0.5 and 1 respectively and plot the kernel density against x.
...
casl_lenet_update_beta_kkt <- function(X, y, b, lambda, active_set, maxit=1000L) {
  if (any(active_set)) {
...
lambda[1], active_set, maxit)
  ...
  {
    lsu <- casl_lenet_update_beta_kkt(X, y, b[,i-1L, drop=FALSE], 
